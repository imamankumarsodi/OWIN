...
protocol CountrySelectedDelegate {
    func SRcountrySelected(countrySelected country: Country,imagePath:String) -> Void
}
...
extension SRCountryPickerController: UITableViewDataSource {
  ...
  func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
    ...
    if checkSearchBarActive() {
        ...
        let countrySelected = countriesFiltered[indexPath.row]
        
                let imagestring = countrySelected.country_code
                let imagePath = "CountryPicker.bundle/\(imagestring).png"
        
        
        countryDelegate.SRcountrySelected(countrySelected: countriesFiltered[indexPath.row], imagePath: imagePath)
    }else {
        let countrySelected = countriesModel[indexPath.row]
        
        let imagestring = countrySelected.country_code
        let imagePath = "CountryPicker.bundle/\(imagestring).png"
        
        countryDelegate.SRcountrySelected(countrySelected: countriesModel[indexPath.row], imagePath: imagePath)
    }
